...
class CartController extends Controller{
    ...
    public function actionAddCart(/*$goods_id,$amount*/){
        ...
        if(\Yii::$app->user->isGuest){//未登录存放在cookie中
            ...
            //实例化模型
            $model=Cart::find()->andWhere(['member_id'=>$member_id])->andWhere(['goods_id'=>$goods_id])->one();
            //var_dump($model);exit;
            if($model){
                \Yii::$app->request->isAjax? $model->amount=$amount: $model->amount+=$amount;
            }else{
                $model = new Cart();
            ...
            if($model->validate()){
                ...
                if(\Yii::$app->request->isAjax){
                    return json_encode(['status'=>'error','msg'=>'失败!']);
                }else{
...
//        $amount=\Yii::$app->request->get('amount');
    ...
    public function actionDelCart($id){
        if(\Yii::$app->user->isGuest){//没登录
            ...
            //var_dump($id);exit;
            $member_id=\Yii::$app->user->identity->id;
            $model =Cart::find()->andWhere(['member_id'=>$member_id])->andWhere(['goods_id'=>$id])->one();
            $model->delete();
        }
        //删除成功，跳转到购物车页面
